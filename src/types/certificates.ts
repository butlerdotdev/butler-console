 ...
 */
export type RotationStatus = 'in-progress' | 'completed' | 'failed' | 'unknown';
...
export interface ClusterCertificates {
  ...
  /** Namespace containing Kamaji TCP resources */
  tcpNamespace?: string;
...
> = {
  all: {
    ...
    description: 'Rotates all non-CA certificates (API server, kubeconfigs, etc.)',
    warning: false,
  },
  kubeconfigs: {
...
export function getSortedCategories(): CertificateCategory[] {
  return (Object.entries(CERTIFICATE_CATEGORIES) as [CertificateCategory, { order: number }][])
...
export function getCategoryHealth(certs: CertificateInfo[]): CertHealthStatus {
  const order: CertHealthStatus[] = ['Healthy', 'Warning', 'Critical', 'Expired'];
